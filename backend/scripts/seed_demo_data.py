...
INTERVIEWS = [
    ...
    ("jad.abou rizk",-12,InterviewStatus.COMPLETED,InterviewType.ONSITE,60,InterviewRecommendation.STRONG_YES,(5,4,5,5)),
...
def ensure_user(db, email, first, last, role, actions, days=120):
    ...
    db.add(user); db.flush(); actions.append(f"create user {email}"); return user
...
def ensure_job_children(db, job, seed, actions):
    ...
    for skill_type, names in ((SkillType.REQUIRED, seed['required']), (SkillType.PREFERRED, seed['preferred'])):
        for name in names:
            if (name.lower(), skill_type.value) not in existing_skills:
                db.add(JobSkill(job_id=job.id, name=name, skill_type=skill_type)); actions.append(f"add skill {name} to {job.title}")
    ...
    for index, text in enumerate(seed['questions'], 1):
        if text.lower() not in existing_questions:
            db.add(ApplicationQuestion(job_id=job.id, question=text, question_type='text', is_required=True, display_order=index)); actions.append(f"add question to {job.title}")
...
def ensure_job(db, seed, hr, actions):
    ...
    if job:
        ...
        ensure_job_children(db, job, seed, actions); return job
    ...
    db.add(job); db.flush(); actions.append(f"create job {seed['title']}"); ensure_job_children(db, job, seed, actions); return job

def ensure_profile_cv(db, user, data, actions):
    first,last,title = data[0],data[1],data[2]
    if not db.scalar(select(ApplicantProfile).where(ApplicantProfile.user_id == user.id)):
        db.add(ApplicantProfile(user_id=user.id, phone=f"+961 70 10{user.id:04d}", location="Beirut, Lebanon", professional_title=title, summary=f"{title} with practical experience and clear communication.", linkedin_url=f"https://linkedin.com/in/{first.lower()}-{last.lower().replace(' ', '-')}-demo")); actions.append(f"create profile {user.email}")
    ...
    if not cv:
        ...
        cv = CV(user_id=user.id, original_filename=f"{first}_{last.replace(' ', '_')}_CV.pdf", stored_filename=stored, content_type='application/pdf', file_size=len(content), is_primary=True, uploaded_at=NOW - timedelta(days=data[5] + 2))
        db.add(cv); db.flush(); actions.append(f"create CV {stored}")
    return cv
...
def ensure_application(db, user, cv, job, data, hr, actions):
    ...
    if app: return app
    submitted = NOW - timedelta(days=data[5])
    app = Application(applicant_id=user.id, job_id=job.id, cv_id=cv.id, status=data[4], submitted_at=submitted, updated_at=submitted, rejection_feedback='Thank you for applying. Other candidates more closely matched the current role requirements.' if data[4] == ApplicationStatus.REJECTED else None)
    db.add(app); db.flush(); actions.append(f"create application {user.email} -> {job.title}")
    ...
    add_history(db, app, data[4], hr.id, submitted)
    return app

def ensure_match(db, cv, app, data, actions):
    analysis = db.scalar(select(CVAnalysis).where(CVAnalysis.cv_id == cv.id))
    if not analysis:
        analysis = CVAnalysis(cv_id=cv.id, extracted_text_preview=f"Demo CV for {data[0]} {data[1]}", professional_summary=f"{data[2]} with {data[13]} years of experience.", skills=data[9]+data[10], technologies=data[9]+data[10], education=[{"degree":"Relevant degree"}], experience=[{"title":data[2],"years":data[13]}], total_experience_years=data[13], job_titles=[data[2]], notable_projects=["Recruitify demo project"], analysis_provider='deterministic-demo')
        db.add(analysis); db.flush(); actions.append(f"create CV analysis {cv.id}")
    if not db.scalar(select(ApplicationMatch).where(ApplicationMatch.application_id == app.id)):
        db.add(ApplicationMatch(application_id=app.id, cv_analysis_id=analysis.id, overall_score=data[14], skills_score=data[6], experience_score=data[7], education_score=data[8], matched_required_skills=data[9], matched_preferred_skills=data[10], missing_required_skills=data[11], relevant_experience=f"{data[13]} years relevant experience.", education_assessment='Education is suitable for the role.', strengths=["Relevant experience", "Clear skill alignment"], gaps=data[11] or ["Continue validating depth during interview"], explanation='Deterministic demo match; no Gemini call was used.')); actions.append(f"create AI match {app.id}")

def ensure_interviews(db, apps, interviewers, hr, actions):
    for i, item in enumerate(INTERVIEWS):
        email = f"{item[0].replace(' ', '')}@{DEMO_DOMAIN}"
        app = apps[email]
        if db.scalar(select(Interview).where(Interview.application_id == app.id)): continue
...
def ensure_contracts(db, apps, hr, actions):
    for prefix,status,ctype,salary,currency,responded_days in CONTRACTS:
        ...
        if db.scalar(select(Contract).where(Contract.application_id == app.id)): continue
        sent_at = NOW - timedelta(days=14); responded_at = NOW - timedelta(days=responded_days) if responded_days is not None else None
...
def seed(db):
    ...
    for data in APPLICANTS:
        email = email_for(data[0], data[1])
        user = ensure_user(db, email, data[0], data[1], UserRole.APPLICANT, actions, data[5] + 7)
        cv = ensure_profile_cv(db, user, data, actions)
        app = ensure_application(db, user, cv, jobs[data[3]], data, hr, actions)
...
def dry_run(db):
    missing_users = sum(1 for d in APPLICANTS if get_user(db, email_for(d[0], d[1])) is None)
...
def main():
    ...
    try:
        if not args.apply:
            ...
            for line in dry_run(db): print('- ' + line)
